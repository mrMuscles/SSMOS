...
public class SheepDisguise extends Disguise {
    ...
    static {
        try {
            Field f = Sheep.class.getDeclaredField("DATA_WOOL_ID");
            f.setAccessible(true);
            DATA_WOOL_ID = (EntityDataAccessor<Byte>) f.get(null);
        } catch (Exception e) {
            throw new RuntimeException("Failed to access Sheep data fields", e);
        }
    }
    ...
    public void setColor(int color_id) {
        ...
        if (color_id == 16) {
            ...
            dataValues.add(SynchedEntityData.DataValue.create(DATA_WOOL_ID, (byte) 0x10));
        } else {
            dataValues.add(SynchedEntityData.DataValue.create(DATA_WOOL_ID, (byte) color_id));
        }
